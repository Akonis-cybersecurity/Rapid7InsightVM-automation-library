...
from aws_helpers.base import AWSConfiguration as AwsModuleConfiguration, AWSModule as AwsModule, OidcAwsMixin
...
class AbstractAwsConnector(OidcAwsMixin, AsyncConnector, metaclass=ABCMeta):
    ...
    def aws_client(self) -> AwsClient:
        """Return an authenticated AWS client using assumed-role credentials."""
        assume_role = self.get_assume_role()
        return AwsClient(assume_role)
